/*
 * Created by JFormDesigner on Thu Jun 01 13:20:42 HKT 2023
 */
 ...
 * @author 17946
 */
public class BookBorrowInter extends JInternalFrame {
    String user_id;
    public BookBorrowInter() {
        initComponents();
        fillBookType("search");
        fillBookType("modify");
        fillTable(new Book());
    }
    public BookBorrowInter(String user_id) {
...
//        JOptionPane.showMessageDialog(null,"该功能暂未实现");
        String book_id = idTxt.getText();
        Borrow borrow = new Borrow(Integer.parseInt(user_id), Integer.parseInt(book_id));
        int add = new BorrowDao().add(borrow);
        if (add > 0){
